...
function Get-SolutionContent {
	...
	Get-ChildItem $packageContentFolder -Directory | ForEach-Object {
		Get-ChildItem $_.FullName -Directory | Where-Object { (Get-ChildItem $_.FullName -Exclude _._).Count -ne 0 } | ForEach-Object {
			...
			try {
			Copy-Item "$($_.FullName)\*" "$SolutionFolder\$($_.Name)\" -Recurse -Force -ErrorAction SilentlyContinue
			} catch {
				Write-Error $_
			}
		}
	...
	Log 'Removing dacpac cache'
	Remove-Item $packageContentFolder\* -Recurse -Force
	Remove-Item $packageContentFolder -Recurse -Force

	Log "Scanning references for $contentFolder data"
	$csPackage = @{}
	try {
		try {
			$folders = Get-ChildItem $solutionFolder -Directory | Where-Object {
				$_.Name -notlike '.*' -and
				$_.Name -notlike "$contentFolder.*" -and
				$_.Name -notin ('PackageTools','PowerShell','Databases','Logs','PackageContent',$contentFolder)
			}
			$configs = $folders | Where-Object {
				Test-Path "$($_.FullName)\packages.config" -ErrorAction SilentlyContinue -ErrorVariable $testPathError
				if ($testPathError)
				{
					Log "Access denied testing $($_.FullName)\packages.config" -Error
					$false
				}
			} | ForEach-Object {
				Get-Item "$($_.FullName)\packages.config"
			}
		} catch {
			Log 'Searching for package.content files failed'
			Write-Error $_
			exit 1
		}
		$configs | ForEach-Object {
			Log "Checking $($_.Directory.Name) references"
			try {
				[xml]$pc = Get-Content $_ -Force
				$pc.packages.package | ForEach-Object {
					New-Object -TypeName PSCustomObject -Property @{ id=$_.id; version=$_.version }
				}
			} catch {
				Log "Reading $_.FullName failed"
				Write-Error $_
			}
		} | Sort-Object -Property id,version -Unique | ForEach-Object {
			if ($csPackage.ContainsKey($_.id)) {
				if ($csPackage[$_.id] -ne $_.version) {
					Log -Warn "$($_.id): replacing version $($csPackage[$_.id]) with $($_.version)"
	...
	Log "Fetching $contentFolder data"
	if ((Test-Path $packageFolder) -and (Get-ChildItem "$packageFolder\**\$contentFolder" -Recurse)) {
		if (Test-Path $solutionContentFolder) {
			...
			Copy-Item -Path $solutionContentFolder\* -Destination $archiveSolutionContentFolder -Recurse -ErrorAction SilentlyContinue
			Log "Clearing $contentFolder"
			Remove-Item -Path $solutionContentFolder\* -Recurse -ErrorAction SilentlyContinue
			[string[]]$exclude = @()
			Get-ChildItem -Path $solutionContentFolder -Recurse -File | ForEach-Object {
				$exclude += $_.name
			}
			if ($exclude.Count -ne 0) {
				Log "$contentFolder files not removed:" -Warn
